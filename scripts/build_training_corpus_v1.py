...
def main() -> None:
    ...
    lines = [
        ...
        "五、适用性说明",
        "--------------",
        "这些样本已经比原始数据库记录更接近大模型训练格式：QA 文件适合问答训练或检索问答评估，SFT 文件适合作为指令格式语料进行整理和复核。但由于样本量仍较小，它目前是 V1 样例语料，不适合直接作为完整训练数据集，只适合验证流水线、报告展示和语料处理迭代。",
        "",
        "六、后续优化方向",
        "----------------",
